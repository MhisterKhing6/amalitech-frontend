            ...
            <section className="raise d-flex flex-column justify-content-center align-items-center">
                ...
                <div>
                    <form className="p-2" onSubmit={async (e) => {
                       ...
                       let url = `/auth/user/verification-number/reset-password/${email}`
                        let response = await getFromBackend(url)
                        if(response.status !== 200) {
                            //dos stuff here
                            alert(response.data.message)
                        } else {
                            let vUrl = "/auth/user/check/verification"
                            let redirectUrl = "/auth/user/update-password"
                            redirect("/auth/verify/customer", {state: {email, verificationId:response.data.verificationId, vUrl, redirectUrl}})
                        }
                        //enable button
                        setSubmitted(false)
                        setSpiner(false)
