    ...
    <div className="container mx-auto d-flex h-100 justify-content-center my-3">
        <form className="registration p-5" onSubmit={async (e) =>{
          ...
          if (registration.password !== registration.cpassword) {
            ...
            //post user data
            let response = await postToBackend("/auth/register/admin", {name:registration.name, password:registration.password, email:registration.email})
          ...
          <h4 className="h3 text-center mb-3 fw-normal my-2 notice"><b>Admin Amalitech</b></h4>
          <hr />
